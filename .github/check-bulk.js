...
// 비교 기준: CI가 넘겨준 푸시 이전 SHA > 원격 master > 직전 커밋
let base = '';
const envBase = (process.env.BULK_BASE || '').trim();
if (envBase && !/^0+$/.test(envBase)) base = sh('git rev-parse --verify --quiet ' + envBase).trim();
if (!base) base = sh('git rev-parse --verify --quiet origin/master').trim();
if (!base) base = sh('git rev-parse --verify --quiet HEAD~1').trim();
if (!base) process.exit(0);   // 첫 커밋이면 검사할 게 없다
if (base === sh('git rev-parse HEAD').trim()) process.exit(0);   // 비교 대상이 자기 자신이면 검사할 게 없다
...
stat.split('\n').forEach(line => {
  ...
  if (!d) return;
  // Windows cmd 에선 2>/dev/null 이 파일 경로로 해석돼 원본 줄수가 0으로 잡힌다 → 리다이렉트 제거(sh 가 실패 시 '' 반환)
  const total = (sh(`git show "${base}:${file}"`).match(/\n/g) || []).length;
  const pct = total > 0 ? d / total : 0;   // 원본을 못 읽으면 비율 판정은 건너뛰고 절대 삭제수만 본다
